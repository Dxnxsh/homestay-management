...
if (!$conn) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formAction === 'pay_deposit') {
      ...
      if (empty($paymentErrors)) {
        ...
        if ($pendingBooking && $pendingBooking['bookingID'] == $selectedPaymentBookingID) {
          ...
          if ($booking_row) {
            ...
            // Apply 10% discount if user has membership OR is buying one now
            $effectiveDiscountRate = $hasMembership ? $discountRate : ($addMembership ? 10.0 : 0.0);
            
            $discountAmount = round($depositAmount * ($effectiveDiscountRate / 100), 2);
            ...
            if ($nextBillNo === null && $bill_insert_result) {
              ...
              if ($bill_insert_result && $booking_insert_result) {
                ...
                $paymentSummary = [
                  'billNo' => $nextBillNo,
                  'bookingID' => $createdBookingID ?? $booking_row['BOOKINGID'],
      ...
      if (empty($bookingErrors) && isset($homestayLookup[$selectedHomestayID])) {
        ...
        // bookingID will be auto-generated later, use -1 for session tracking
        $nextBookingID = -1;
  ...
  if (isset($_SESSION['pending_booking'])) {
    // Fix for stale sessions: Ensure bookingID is -1 if null/empty so it works in payment form
    if (empty($_SESSION['pending_booking']['bookingID'])) {
      $_SESSION['pending_booking']['bookingID'] = -1;
    }
...
<body>
            ...
            <?php if ($bookingSummary): ?>
              <div class="booking-success-summary">
                <div class="summary-row">
                  ...
                  <span><?php echo $bookingSummary['bookingID'] == -1 ? 'Pending' : '#' . htmlspecialchars($bookingSummary['bookingID']); ?></span>
                </div>
                <div class="summary-row">
                  <span>Homestay</span>
                    ...
                    <select name="bookingID" id="paymentBookingID" data-scroll-onload="<?php echo $shouldPromptPayment ? '1' : '0'; ?>" required>
                      ...
                      ?>
                        <option value="<?php echo $booking['bookingID']; ?>"
                          ...
                          <?php echo $booking['bookingID'] == -1 ? "New Booking" : "#" . $booking['bookingID']; ?> · <?php echo htmlspecialchars($booking['homestay_name']); ?> (<?php echo $checkinInline; ?> - <?php echo $checkoutInline; ?>) · <?php echo $depositLabel; ?>
                        </option>
                      <?php endforeach; ?>
                    </select>
                  </div>
                ...
                <div class="form-group">
                  ...
                  <div class="select-wrapper">
                    <select name="payment_method" id="payment_method" required>
                      ...
                      <option value="Credit/Debit Card" <?php echo ($paymentMethodInput === 'Credit/Debit Card') ? 'selected' : ''; ?>>Credit / Debit Card</option>
                      <option value="QR Payment" <?php echo ($paymentMethodInput === 'QR Payment') ? 'selected' : ''; ?>>QR Payment</option>
                    </select>
                  </div>
                </div>
  ...
  <script>
    ...
    // Calendar Logic - Defined globally to be accessible
    const renderCalendar = () => {
      // Guard: If calendar elements are missing (e.g. Step 2), do nothing
    ...
    // Calendar & Step 1 Logic Initialization
    if (dateRangeInput && calendarPopup && monthYearEl && calendarDaysEl) {
        // Initialize calendar
        renderCalendar();
        ...
        if (checkinInputEl && checkoutInputEl && checkinInputEl.value && checkoutInputEl.value) {
    ...
    // Step 1: Homestay Selection Logic
    if (homestaySelect) {
        ...
        if (selectedHomestayValue) {
          ...
          if (dateRangeInput) dateRangeInput.disabled = false;
          if (adultsInputEl) adultsInputEl.disabled = false;
          if (childrenInputEl) childrenInputEl.disabled = false;
        ...
        homestaySelect.addEventListener('change', (e) => {
          ...
          if (dateRangeInput) dateRangeInput.disabled = !hasSelection;
          if (adultsInputEl) adultsInputEl.disabled = !hasSelection;
          if (childrenInputEl) childrenInputEl.disabled = !hasSelection;
    ...
    const updatePaymentSummary = () => {
      ...
      // Use 10% if membership is being bought now, otherwise use existing rate
      const effectiveRate = (addMembershipCheckbox && addMembershipCheckbox.checked) ? 10 : inlineDiscountRate;
      
      const discountValue = deposit * (effectiveRate / 100);
    ...
    // Auto-trigger update on load if a value is selected
    if (paymentBookingSelect && paymentBookingSelect.value) {
      // Small delay to ensure DOM is ready
      setTimeout(() => {
        paymentBookingSelect.dispatchEvent(new Event('change'));
        
        // Only scroll if explicitly requested
        if (paymentBookingSelect.dataset.scrollOnload === '1') {
          scrollToPaymentSection();
        }
      }, 100);
    }
